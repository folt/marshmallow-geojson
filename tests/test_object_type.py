...
class TestGeoJSONType:
    """Test suite for GeoJSONType enum."""

    @pytest.mark.parametrize(
        "enum_member,expected_value",
        [
            (object_type.GeoJSONType.point, object_type.POINT),
            (object_type.GeoJSONType.multi_point, object_type.MULTI_POINT),
            (object_type.GeoJSONType.line_string, object_type.LINE_STRING),
            (object_type.GeoJSONType.multi_line_string, object_type.MULTI_LINE_STRING),
            (object_type.GeoJSONType.polygon, object_type.POLYGON),
            (object_type.GeoJSONType.multi_polygon, object_type.MULTI_POLYGON),
            (object_type.GeoJSONType.geometry_collection, object_type.GEOMETRY_COLLECTION),
            (object_type.GeoJSONType.feature, object_type.FEATURE),
            (object_type.GeoJSONType.feature_collection, object_type.FEATURE_COLLECTION),
        ],
    )
    def test_geo_json_type_values(self, enum_member, expected_value):
        """Test that GeoJSONType enum members have correct values."""
        assert enum_member.value == expected_value

    def test_geo_json_type_length(self):
        """Test that GeoJSONType enum has correct number of members."""
        assert len(list(object_type.GeoJSONType)) == 9

    def test_geo_json_type_iteration(self):
        """Test that GeoJSONType can be iterated."""
        types = list(object_type.GeoJSONType)
        assert len(types) == 9
        assert all(hasattr(t, "value") for t in types)
